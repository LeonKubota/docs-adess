...
= Úvod
Cílem tohoto projektu je vytvořit program, který dokáže na základě uživatelských pokynů generovat zvuk motorů. Cílem není generovat zvuk živě, ale generovat jej pro následné využití ve filmu a animaci.\

V současnosti se pro tvorbu zvuku motorů využívá nahrávání skutečných motorů a následná digitální manipulace. Tento přístup umožňuje nejvyšší kvalitu zvuku, je však velice náročný i nákladný. Tento projekt nabízí alternativu pro menší a amatérské produkce, které si předešlý způsob tvorby zvuku nemohou dovolit. @MIX:EngineFXWithPersonalityInPixarsCars\

Rozhodl jsem se zvuk tvořit synteticky, přestože je v současnosti hojně využívána simulace. Výhodou syntézy je násobně kratší doba obratu a vyšší míra ovládání, uživatel tedy může rychleji a více experimentovat. Tento přístup je také možné značně urychlit paralelizací, jelikož obsahuje mnoho na sobě nezávislých prvků. @GH:EngineSim @IEEE:EngineSoundSynthesis

Aplikace se ovládá skrze nastavovací soubory, ve kterých uživatel charakterizuje vlastnosti motoru, přeměnu hodnot skrze čas a parametry výstupního souboru `WAVE`. Tyto nastavovací soubory jsou zapisovány ve vlastním "jazyce" pomocí párů klíč -- hodnota. Pro generaci zvuku použije uživatel příkaz `adess render` a název scény, aplikace poté vypočítá zvukové vzorky a uloží je. Proces renderování je velice rychlý, vytvořit šedesátisekundovou zvukovou stopu trvá přibližně jednu sekundu.\

Vedlejším cílem bylo blíže porozumět fungování počítače a jeho programování. Tento vedlejší cíl je jeden z důvodů, které vedly k tvorbě tohoto projektu v jazyce `C`. Nejprve byl však projekt započat v jazyce `Python` pro rychlou experimentaci, poté v jazyce `Java`. Teprve po tomto prvotním průzkumu byl program napsán v jazyce `C`.\

Aplikace `Adess` je také využitelná jako jakási nástavba pro komplexnější nástroje, které mohou uživateli zjednodušit nastavování parametrů pomocí grafického rozhraní.
...
= Ovládání aplikace `Adess`
Aplikace se ovládá skrze příkazy psané v příkazové řádce a nastavovací soubory. V Tab. @TAB:commands[] jsou popsány příkazy dostupné v aplikaci `Adess`.
...
#figure(
  table(
    ...
    [`adess help`], [bez přepínačů], [Zobrazí uživateli možnosti aplikace `Adess`.],
  ...
  caption: [Přehled příkazů aplikace `Adess`],
) <TAB:commands>
...
#figure(
  ...
  caption: [Přehled přepínačů aplikace `Adess`],
) <TAB:options>

== Parsování příkazů
Při spuštění načte aplikace argumenty do dvojitého ukazatele typu `char` (v kódu: `**argv`). Řetězec s indexem 1 je porovnán s názvy příkazů a pokud dojde ke shodě, příkaz je spuštěn pomocí funkce `execute`. Ta zavolá požadovanou implementaci příkazu pomocí ukazatele na funkci. Kontrola vstupu je provedena na úrovni jednotlivých příkazů v příslušné funkci.\

Parsování přepínačů je komplexnější, jelikož některé přepínače požadují hodnotu (implementovány jsou i parametry s několika hodnotami, ty však nejsou využity). K zjišťování vlastností přepínaču je využit tento řetězec: `"hn.d.ep"`. Jednotlivá písmena představují možné přepínače, pokud se za písmenem vyskytuje znak `'.'`, jde o parametr s hodnotou (několik hodnot je označeno znakem `':'`). Aktivní parametry se ukládají do pole `bool g_opts[8]`, jejich hodnoty jsou uloženy v poli řetězců `char *g_vals[8][2]`. Tyto globální proměnné jsou později využity funkcemi, které implementují příkazy.\
...
Syntax nastavovacích souborů je velice jednoduchý. Jsou podporovány 3 datové typy: celá čísla (`integer`), desetinná čísla (`float`) a řetězce (`string`). Čísla jsou omezena na nezáporná, ve výpisu @C:syntax[] je ukázka nastavovacího souboru (formát také podporuje komentáře).
...
Povolené proměnné projektového souboru jsou popsány v Tab. @TAB:project[].
...
#figure(
  table(
    ...
    [`bit_depth`], [`integer`], [Rozlišení vzorku ($8$, $16$, $24$ či $32$ _bitů_).],
...
Soubory scén obsahují informace o scéně. Možné hodnoty jsou zapsány v Tab. @TAB:scene[].
...
Parsování souborů by bylo možné i pomocí rozdělení vstupního souboru na tokeny a jejich následné čtení. Tento přístup je jednodušší a robustnější než přístup v `Adess`, který je poněkud neohrabaný. V budoucnosti bude parsování souborů upraveno z důvodů zpřehlednění kódu a podpory operačního systému `Windows`, pravděpodobně však v programovacím jazyce `Odin`.
...
#figure(
  cetz.canvas({
    import cetz.draw: *
    circle((0, 5.25), radius: 0.01cm, stroke: white)
...
== Předvýpočetní fáze
V této fázi jsou předvypočítány důležitá pole pro následující fáze. Tyto výpočty probíhají paralelně.
...
$t$ je čas, $t_0$ a $t_1$ jsou časy předchozího a následujícího klíčového snímku [s]\

Frekvence je vypočítána pomocí otáčkové rovnice (@RV:otacky[]).
$ f = (o t) / (60 n) dot v $ <RV:otacky>
$f$ je frekvence [Hz]\
$o t$ jsou otáčky [ot $dot$ min#super[-1]]\
...
$phi_n$ je fáze ve vzorku $n$ (čas je převeden na vzorky) [-]\
$n$ je index současného vzoreku [-]\
$f[i]$ je frekvence ve vzorku $i$ [Hz]\
$Delta t$ je změna času oproti minulému vzorku (je rovna převrácené hodnotě vzorkovací frekvenci) [s]
...
$s$ je síla šumu [-]\
$o t$ a $o t_v$ jsou okamžité otáčky a otáčky ve volnoběhu [ot $dot$ min#super[-1]]\
...
Hnědý šum je tvořen _Brownovým pohybem_, lze získat integrováním bílého šumu. V aplikaci je využit jako obecný zdroj náhodnosti ve většině funkcí, jeho generace je ve výpisu @C:BrownNoise[]. @WIKI:BrownianNoise\
...
Hodnoty jsou dále stabilizovány a vyhlazeny dle _Gaussova rozdělení_.
...
Pro generaci růžového šumu jsem zvolil _Voss-McCartneyův_ algoritmus, opět s použitím $32$ _bitového_ `xorshift` algoritmu, který je však ve výpisu @C:PinkNoise[] vynechán.
#figure(
  code(
    ```C
    while (i < scene->sampleCount) {
...
Nízkofrekvenční šum je využit při generaci základního zvuku motoru ve volnoběhu. Opět využívá $32$ bitový `xorshift`, jeho výstupní hodnotu využívá pro změnu fáze, která je poté přepočítána na vzorek.
...
Tato stopa odpovídá zvuku spalování v motoru. Vzorky stopy jsou vypočítány pomocí předvypočtené fáze, výsledná zvuková stopa je zachycena ve spektogramu @OBR:spek[]. Při generaci jsou také přidány harmonické frekvence.
...
$ n = 10 dot [abs(sin(0,1 p + phi_s)) - 9] $ <RV:pulzy>
$n$ je momentální hodnota vzorku modulační vlny [-]\
$phi_s$ je fáze otevírání sacího ventilu [-]\
...
=== Tónový posun
Takto převádět zvukovou stopu do frekvenční dimenze lze pouze v případě, že frekvence zůstane konstantní. V případě, že je frekvence proměnná, je zapotřebí rozdělit zvukovou stopu na malé části, ve kterých je frekvence téměř konstantní. Tyto části (dále okna) se překrývají, aby se po jejich složení mohly roztáhnout mezery mezi nimi (a tak se zvýšila frekvence bez zrychlení zvukové stopy).

=== Potlačení spektrálního přelivu
Okna však z důvodu nesynchronizovaných fází nenavazují a vzniká nežádoucí bzučení (anglicky _spectral leakage_), k omezení tohoto jevu je využito _Hanningovo_ okno (viz rovnice (@RV:Hanning[])), které tyto náhlé přechody vyhlazuje. V současnosti se pro synchronizaci fází komplexní fázové zarovnávače, dále se zkoumá využití umělé inteligence.

$ w_H(n) = cases(0\,5 dot [1 - cos(tau n / N)]\; quad 0 <= n = N - 1 , quad quad quad quad quad quad 0 quad "jinak") $ <RV:Hanning>

Celý proces tónového posunu je vykonán čtyřikrát (paralelně), výsledné posunuté vlny jsou zkombinovány, jejich amplitudy jsou s rostoucí frekvencí snižovány.
...
Další možnou cestou pro tento projekt je využití simulace namísto syntézy. Atmosféru lze v tomto případě považovat za ideální plyn, což znamená, že na sebe částice vzájemně silově nepůsobý. Pohyb částic by tedy mohl být paralelně vypočítáván pomocí grafické karty. Následně by byl v této 3d scéně umístěn digitální mikrofon, který by snímal simulovaný tlak. Takováto simulace by trvala řádově déle, umožnila by však tvořit zvuk, který působý organicky a skutečně.
...
Díky tomuto projektu jsem měl možnost zkombinovat několik různých disciplín: matematiku (_Fourierova_ transformace), fyziku (zvuk), inženýrství (pro znalost spalovacích motorů) a samozřejmě programování. Dále bylo dosaženo vysoké rychlosti pomocí drobných optimalizací a především paralelizace.
